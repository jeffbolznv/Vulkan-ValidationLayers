...
void PositiveGpuAVPoisonValue::SimpleComputeTest(const char* shader, SpvSourceType source_type) {
    InitPoisonValue();
...
TEST_F(PositiveGpuAVPoisonValue, VariablePointersStorageBuffer) {
    ...
    // Hand-written SPIR-V: OpSelect chooses between two SSBO element pointers
    // based on a clean (initialized) condition, then stores through the result.
    const char* cs_source = R"asm(
               OpCapability Shader
